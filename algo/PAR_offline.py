...
class MLPNetwork(nn.Module):
    ...
    def __init__(self, input_dim, output_dim, hidden_size=256, layer_norm=False):
...
class DoubleQFunc(nn.Module):
    ...
    def __init__(self, state_dim, action_dim, hidden_size=256, layernorm=False):
        super(DoubleQFunc, self).__init__()
        self.network1 = MLPNetwork(state_dim + action_dim, 1, hidden_size, layernorm)
        self.network2 = MLPNetwork(state_dim + action_dim, 1, hidden_size, layernorm)
...
class PAR(object):
    ...
    def __init__(self,
        ...
        # aka critic
        self.q_funcs = DoubleQFunc(config['state_dim'], config['action_dim'], hidden_size=config['hidden_sizes'], layernorm=self.layernorm).to(self.device)
    ...
    def update_policy_and_temp(self, state_batch, action, src_state, src_action):
        ...
        pred_src_act, _, _ = self.policy(src_state, get_logprob=True)
        policy_loss = p_w * (self.alpha * logprobs_batch - qval_batch).mean() + F.mse_loss(pred_src_act, src_action)
    ...
    def train(self, src_replay_buffer, tar_replay_buffer, batch_size=128, writer=None):
        ...
        # update encoder, vanilla update interval 200
        if self.total_it % 200 == 0:
            tar_s, tar_a, tar_ns, _, _ = tar_replay_buffer.sample(batch_size)
            self.update_encoder(tar_s, tar_a, tar_ns, writer)
        
        # derive representation deviation
        with torch.no_grad():
            next_src_zs = self.encoder.zs(src_next_state)
            src_zs = self.encoder.zs(src_state)
            pred_src_zs = self.encoder.zsa(src_zs, src_action)
        ...
        for p in self.q_funcs.parameters():
            p.requires_grad = False
        pi_loss_step, a_loss_step = self.update_policy_and_temp(state, action, src_state, src_action)
